...
\begin{thm}{Proposition}\label{prop:parallel}
Let $\Sigma$ be a smooth surface in the Euclidean space and $\gamma\:[a,b]\z\to \Sigma$ be a smooth curve.
...
so we can write $\vec w(b)=\iota_\gamma({\vec w}(a))$ or $\vec w(b)=\iota_\gamma({\vec w}(a))_\Sigma$ if we need to emphasize that $\gamma$ lies in the surface~$\Sigma$.
...
so $\gamma(t)\z=s(u(t),v(t))$ for some smooth functions $t\mapsto u(t)$ and $t\mapsto v(t)$.
Set 
\[
\vec u(t)=s_u(u(t),v(t)),
\quad
...
Applying the statement consequently to each $\gamma_i$ proves it for $\gamma$.
\qeds
...
Suppose $\gamma\:[a,b]\to\Sigma$ is a smooth regular arc in a smooth surface $\Sigma$.
Think of walking along $\gamma$ and carrying a perfectly balanced \emph{horizontal} bike wheel.
More precisely, you keep wheel's axis normal to $\Sigma$ and touch only the tips of the axis.
...
Observation \ref{obs:parallel=} essentially states that \textit{moving the axis of the wheel without changing its direction does not change the direction of the wheel's spokes.}
...
Note that if $\Sigma$ is a plane, then the geodesic curvature of $\gamma$ coincides
