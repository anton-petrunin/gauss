...
Note that to prove Proposition \ref{prop:convex-curve} it is sufficient to show that perimeter of any polygon inscribed in $\alpha$ is not longer that  length of $\beta$.
...
\parit{Proof.}
Suppose $F'$ is a figure obtained from $F$ by cutting it along a \emph{chord};
that is, a line segment in $F$ that runs from boundary to boundary.
...
Observe that there is an decreasing sequence of figures 
\[F=F_0\supset F_1\supset\dots\supset F_n=P\]
such that $F_{i+1}$ obtained from $F_{i}$ by cutting along a chord.
Therefore, 
\begin{align*}
