...
\parit{Proof.}
The short arc $\gamma$ of a great circle\footnote{A great circle is a circle of radius 1 contained in $\mathbb{S}^2$. Equivalently, a subset $C$ of $ \mathbb{S}^2$ is a great circle if there is an isometry of $\mathbb{S}^2$ that sends the equator $ \mathbb{S}^2 \cap \{ z = 0 \}$ to $C$.}  from $u$ to $v$ in $\mathbb{S}^2$ has length $\measuredangle(u,v)$.
Therefore,
