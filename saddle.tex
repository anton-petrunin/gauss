...
If this happens, then $\Sigma_1$ and $\Sigma_2$ are tangent at $p$, and after adjusting the orientation of one of them, we can assume they are cooriented.
...
Note that $\Sigma_2$ locally supports $\Sigma_1$ at $p$ if and only if either
\[   f_1(x,y)\ge f_2(x,y)
\quad\text{or}\quad
f_1(x,y)\le f_2(x,y)\]
...
The proof is guided in the following exercise.
