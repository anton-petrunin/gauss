\begin{thm}{Advanced Exercise}\label{ex:pogorelov}
Let $f\:\Sigma\to \Sigma^*$ be an intrinsic isometry between open strictly convex surfaces.
Suppose that the equality
\[|p-x|=|f(p)-f(x)|\]
holds for a fixed point $p\in \Sigma$ and any other point $x\in \Sigma$.
Prove that $f$ is a restriction of an Euclidean space motion to the surface $\Sigma$.
\end{thm}
...
Choose $q\in \Sigma$, let $z$ be its $z$-coodinte.
Denote by $z^*$ the $z$-coodinte of $f(q)$.
Show that $z^*\le z$.

Swipping the roles of $\Sigma$ and $\Sigma^*$, we also get that $z^*\ge z$;
that is $z=z^*$ for any $q\in \Sigma$.
