...
Show that if $\bar B[p,2]\subset \bar B[q,1]$ for $p,q\in \spc{X}$, then $\bar B[p,2]\z=\bar B[q,1]$.
\end{subthm}
...
\begin{thm}{Definition}
 Let ${\spc{X}}$ be a metric space.
A sequence of points $x_1, x_2, \ldots$ in ${\spc{X}}$ \index{convergence of points}\emph{converges}
if there is a point
...
Moreover, if $\phi$ is bounded above and/or below by constants, then we may assume that so is~$\psi_\epsilon$.  
\end{thm}
...
The Jacobian matrix of $\bm{f}$ at $\bm{x}\in\mathbb{R}^m$ is defined as \index{10j@$\Jac$}
\[\Jac_{\bm{x}}\bm{f}=
\begin{pmatrix}
