...
\section{Isometries and motions} 
...
Two metric spaces are called \index{isometric spaces}\emph{isometric} if there exists an isometry between them.

An isometry from a metric space to itself is also referred a \index{motion}\emph{motion}.
...
that is, if $f\:\spc{X}\to\spc{Y}$ is a distance-preserving map, 
...
A subset $K$ of a metric space is called \index{compact}\emph{compact} if any sequence $x_n$ of points in $K$ has a subsequence that converges to a point $x_\infty$ in~$K$.
...
then we may also say that $\spc{X}$ is a \index{topological}\emph{topological} plane, sphere, disc, circle, and so on.
...
A set $X$ in the Euclidean space is called \index{connected}\emph{connected} if one cannot cover $X$ with two disjoint open sets $V$ and $W$ such that both intersections $X\cap V$ and $X\cap W$ are nonempty.
...
A function is called \index{measurable function}\emph{measurable} if the inverse image of any measurable set is measurable.
...
Here we construct examples of smooth functions that mimic the behavior of certain model functions.
...
The Jacobian matrix of $\bm{f}$ at $\bm{x}\in\mathbb{R}^m$ is defined as \index{10j@$\Jac$ (Jacobian matrix)}
\[\Jac_{\bm{x}}\bm{f}=
\begin{pmatrix}
...
The \index{inverse function theorem}\emph{inverse function theorem} gives an if condition for a smooth map $\bm{f}$ to be invertible in a neighborhood of a given point $\bm{x}$.
