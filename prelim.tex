...
A point $p$ lies on the \index{boundary}\emph{boundary} of a set $Q$ (briefly, $p\in\partial Q$) if any neighborhood of $p$ contains points in $Q$ an in the complement of~$Q$.
...
These definitions and hyperplane separation should appear on the first pages of any introductory text in convex geometry;
...
This equation can be rewritten as $k$ first-order equations  with $k-1$ new vector variables $\bm{y}_i=\bm{x}^{(i)}$:
\[
\begin{cases}
\bm{x}'&=\bm{y}_1,
\\
