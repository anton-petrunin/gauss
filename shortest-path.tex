...
A property is called \index{intrinsic}\emph{intrinsic} if it can be checked by measurements within the surface;
for example, lengths of curves or angles between the curves that lie on the surface.
Otherwise, if a property's definition essentially uses the ambient space, such property is called \index{extrinsic property}\emph{extrinsic}.
...
However, the mean curvature of the former vanishes at all points, while the mean curvature of the latter does not vanish, say at $p=(0,0,1)$.  
...
The image of a shortest path between $p$ and $q$ in $\Sigma$ is usually denoted by $[p,q]$ or $[p,q]_\Sigma$.\index{10aac@$[p,q]$, $[p,q]_\Sigma$ (shortest path)}
...
Note that if $\measuredangle \hinge{\bar p}{p}{\bar q}< \tfrac\pi2$, then $\dist{p}{x}{}\z<\dist{p}{\bar p}{}$ for a point $x\in [\bar p,\bar q]$ close to $\bar p$.
