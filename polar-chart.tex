...
More precisely, by Jordan’s theorem (\ref{thm:jordan}), $\Delta$ can be parametrized by the standard disc, or equality the unit square.
\begin{figure}[!ht]
\vskip-0mm
\centering
...
\vskip-0mm
\end{figure}
Applying the subdivision shown on the diagram, sufficiently many times we can assume the formula holds for any small square.
Note that on each step we divide each disc in two;
therefore, applying \ref{lem:GB-sum} several times we get the formula for the big square.
...
By \ref{eq:gb(n)}, $f(t)$ is a multiple of $2\cdot\pi$ for any~$t$.
