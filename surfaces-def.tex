...
if any point $p\in \Sigma$ admits a neighborhood $W$ in $\Sigma$
that can be parametrized by an open subset in the Euclidean plane; 
that is, there is a homeomorphism $V\to W$ from an open set $V\subset \mathbb{R}^2$; see Appendix~\ref{sec:topology}.
...
Therefore, a smooth surface is an example of a topological surface.
...
that is, there is an open set $V\subset \mathbb{R}^3$ such that $s\circ w(W)=V\cap \Sigma$ is a graph of a smooth function.
