...
For a surface $\Sigma$ and $k \in \mathbb{N}$, a map $f\: \Sigma \to \mathbb{R}^k$ is said to be \emph{smooth} if for any chart $s \: U\to \Sigma$,
the composition $f\circ s$ is smooth; that is, all partial derivatives $\frac{\partial^{m+n}}{\partial u^m\partial v^n}(f\circ s)$ are defined and continuous in the domain of definition. Further, a smooth map $f \: \Sigma \to \mathbb{R}^3$ is called a 
