...
Further, we will use the term \index{circline}\emph{circline} for a \textit{circle or a line};
...
The unit-speed curve $\sigma_s$ of constant signed curvature $\skur(s)$ that starts at $\gamma(s)$ in the direction $\gamma'(s)$ is called the \index{osculating!circline}\emph{osculating circline} of $\gamma$ at~$s$.
...
then $\sigma_{s_0}$ lies in the open disc bounded by $\sigma_{s_1}$ for any $s_0<s_1$. \index{10sigma@$\sigma_s$ (osculating circline)}
\end{thm}
