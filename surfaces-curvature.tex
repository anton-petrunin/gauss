...
The values $\ell$, $m$, and $n$ are completely determined by this equation.\index{10lmn@$\ell$, $m$, $n$ (Hessian-matrix components)}
The graph of Taylor polynomial
\[z=\tfrac12\cdot(\ell\cdot x^2+2\cdot m\cdot x\cdot y+n\cdot y^2)\]
is called \index{osculating!paraboloid}\emph{osculating paraboloid}.
It has \index{order of contact}\emph{second-order contact} with $\Sigma$ at~$p$.
...
\eqlbl{eq:hessian}
\]
is the so-called \index{Hessian matrix}\emph{Hessian matrix} of $f$ at $(0,0)$.\index{10m@$M_p$ (Hessian matrix)}

\section{Principal curvatures}\label{sec:Principal curvatures}

The tangent-normal coordinates are almost canonical; they depend on the point $p$ and are unique up to a rotation of the $(x,y)$-plane.
...
The mean curvature can also be interpreted as the trace of the Hessian matrix $M_p\z=
(\begin{smallmatrix}
\ell&m
\\
m&n
...
Given $\vec w\in \T_p$,
the \index{shape operator}\emph{shape operator}%
\footnote{
...
Therefore, $\Shape_p$ is indeed a linear operator $\T_p\Sigma\to \T_p\Sigma$ (the latter also will follow from \ref{thm:shape-chart}).

The value of the shape operator on a tangent vector $\vec w\in \T_p\Sigma$ can be denoted by $\Shape\vec w$
if it is clear from the context which base point $p$ and which surface $\Sigma$ we are working with;
...
\end{align*}
where $\vec i,\vec j,\vec k$\index{10i@$\vec i$, $\vec j$, $\vec k$ (standard basis)} is the standard basis in $\mathbb{R}^3$.
...
Let $\Sigma$ be a smooth surface whose Gaussian curvature never vanishes.
Assume that its spherical map $\Norm\:\Sigma\to\mathbb{S}^2$ is injective.
Then
...
Since $\Shape_p=-d_p\Norm$, it follows from \ref{cor:Shape(ij)} that
\[
\jac_p\Norm
=
\left|\det\bigl(\begin{smallmatrix}
k_1 & 0\\
0   & k_2
\end{smallmatrix}\bigr)\right|
= |K(p)| \ne 0
\]
for every point $p\in\Sigma$.
In particular, the map $\Norm$ is regular and, since it is injective,
it defines a diffeomorphism $\Sigma\to\Norm(\Sigma)$.
It remains to apply the area formula (\ref{prop:surface-integral}).
\qeds
...
Show that the point $q=p+\Norm(p)$ does not depend on $p\in\Sigma$.
...
Since the frame $\tan(s),\mu(s),\Norm(s)$ is orthonormal, these curvatures can also be written as the following scalar products:
\begin{align*}
...
\parbf{Proof.} Since $\gamma''=\tan'=\kur\cdot \norm$, we get that
\begin{align*}
...
In this case, the curve \( \gamma \) is called a \index{tractrix}\emph{tractrix}, and the surface is called a \index{pseudosphere}\emph{pseudosphere};
...
Show that the \index{helicoid}\emph{helicoid} defined by the following parametrization
...
The following question is a 3-dimensional analog of the moon in a puddle problem (\ref{thm:moon-orginal}).
...
\parbf{Construction.}
Let us start with a body of revolution $V_1$ whose cross-section is shown on the next picture.
...
(To make the curves smooth, one has to use cutoffs and mollifiers from Appendix~\ref{sec:analysis}.)
...
this and more was proved by Vladimir Lagunov and Abram Fet \cite{lagunov-1960, lagunov-1961, lagunov-fet-1963, lagunov-fet-1965}.
...
Try to make the boundary surface a sphere with $3$ handles.
\end{thm}
