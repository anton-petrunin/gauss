...
This is not the case for second-order approximations, which are determined by the principal curvatures.
...
Since $M_p$ is symmetric, by the spectral theorem (\ref{thm:spectral}) it is diagonalizable by orthogonal matrices.
...
If we reverse the orientation of $\Sigma$, then the principal curvatures switch their signs and indexes.
(One may say that $k_1$ becomes $-k_2$ and $k_2$ becomes $-k_1$).
...
\parit{Proof.}
Note that $s\:(u,v)\mapsto (u,v,f(u,v))$ is a chart of $\Sigma$ at~$p$.
Furthermore, 
\begin{align*}
s_u(0,0)&=\vec i,
&
...
Let $\Sigma$ be a smooth surface with orientation defined by a unit normal field $\Norm$,
and let $Z\subset \Sigma$ be a connected set with vanishing shape operator.
Show that $Z$ lies in a plane.
\end{thm}
...
we will show that the answer is ``yes'' for convex sets.
