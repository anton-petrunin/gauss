...
The following exercise is the key part in the proof \cite{massey1962} of the following fact:
\textit{Any open surface with vanishing Gauss curvature a \index{cylindrical surface}\emph{cylindrical surface}};
the latter means that the surface is swept out by a family of parallel lines.

\begin{thm}{Advanced exercise}\label{ex:flat-plane}
Let $\Sigma$ be a smooth oriented surface and $(u,v)\mapsto s(u,v)$ be its chart defined in a coordinate rectangle.
...
the principal curvature in the direction $\vec u$ vanishes 
and $k=k(u,v)$ is the principal curvature in the direction $\vec v$ at $s(u,v)$.
...
Show that for fixed $v$ the value $\tfrac1k$ depends linearly on $u$.
\end{subthm}
...
with the generatrix $y=a\cdot \sin x$ for a constant $a>0$ and $x\in (0,\pi)$.
