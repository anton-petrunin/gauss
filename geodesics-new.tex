...
The following proposition formalizes that the motion on a surface without friction and external forces depends smoothly on its initial data. 
...
\parit{Proof.} The first condition $z(t)=f(x(t),y(t))$ simply means that $\gamma(t)$ lies on the graph $z=f(x,y)$.
...
that is, the maximal interval $\mathbb{I}$ is a proper subset of the real line.
...
Moreover, we have {}\emph{local control} on the function $p\mapsto r_p$;
that is, there is $\epsilon>0$ such that if $\dist{p}{q}\Sigma<\epsilon$ for some $q\in \Sigma$ , then $r_q\ge\epsilon$.
\end{thm}
...
Denote by $h$ the composition of the exponential map $\exp_p$ with the orthogonal projection $(x,y,z)\mapsto (x,y)$.
...
Denote by $h_q$ the composition of the exponential map $\exp_q$ with the orthogonal projection $(x,y,z)\mapsto (x,y)$.
...
The least upper bound on $r_p$ that satisfies \ref{prop:exp} is called the \index{injectivity radius}\emph{injectivity radius} of $\Sigma$ at $p$;
...
In fact, \textit{the function $\inj\:\Sigma\to (0,\infty]$ is continuous};
...
Consider the graph $z=\ell\cdot\sqrt{x^2+y^2}$ with the origin removed;
it is a smooth surface; denote it by~$\Sigma$.
...
We can assume that the geodesic $\gamma$ does not enter the smoothed part of the graph.
