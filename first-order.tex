...
Construct two functions $u$ in a neighborhood of $p$ such that 
\begin{align*}
D_{\vec x} u&>0,
&
D_{\vec y} u&=0,
...
Conclude that there is a chart $(u,v)\mapsto s(u,v)$ of $\Sigma$ at $p$ such that $\vec x$ and $\vec y$ are tangent to the coordinate lines; in other words, $\vec x$ and $\vec y$ are proportional to $s_u$ and $s_v$ at any point.
\end{thm}
...
is called the \index{differential}\emph{differential} of $s$ at~$p$.
...
does not depend on the choice of orthonormal bases in $\T_p$ and $\T_{s(p)}\Sigma_1$. \label{page:|L|}\index{10d@$d_p f$ (differential)}

Let $ s_1\:\Sigma_1\to\Sigma_2$ be a smooth map between smooth surfaces $\Sigma_1$ and $\Sigma_2$.
Suppose ${p_1}= s_0(p_0)\in\Sigma_1$ for some chart $s_0$ of $\Sigma_1$;
observe that 
\[d_{p_0}( s_1\circ s_0)=d_{p_1} s_1 \circ d_{p_0} s_0 .\]
It follows that
\[\jac_{p_0}( s_1\circ s_0)
=
\jac_{p_1} s_1\cdot\jac_{p_0} s_0 .\eqlbl{eq:jac-composition}\]


The value $\jac_{p_0} s_0$ can be found using the following formulas:
\begin{align*}
\jac s_0
&=|(s_0)_v\times (s_0)_u|=
\\
&=\sqrt{\langle (s_0)_u, (s_0)_u\rangle\cdot\langle (s_0)_v, (s_0)_v\rangle -\langle (s_0)_u, (s_0)_v\rangle^2}=
\\
&=\sqrt{\det[\Jac^\top s_0\cdot \Jac s_0 ]},
\end{align*}
where $\Jac s_0$ denotes the Jacobian matrix of $s_0$; it is a $2{\times}3$ matrix with column vectors $(s_0)_u$ and $(s_0)_v$.

The value $\jac_{p_1} s_1$ has the following geometric meaning:
if $P_1$ is a region in $\T_{p_1}$ and $P_2=(d_{p_1} s_1)(P_1)$, then
\[\area P_2=\jac_{p_1} s_1 \cdot \area P_1.\]
