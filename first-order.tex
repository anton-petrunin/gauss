...
Then the directional derivative of $f$ at $p$ along $\vec w$ is defined as \index{10d@$D_{\vec{w}}f$}
\[D_{\vec w}f(p)\df h'(0).\]
...
does not depend on the choice of orthonormal bases in $\T_p$ and $\T_{s(p)}\Sigma_1$. \label{page:|L|}\index{10d@$d_p f$}
...
The \index{region}\emph{region} is defined as any Borel set $R\subset\Sigma$,
...
each connected component of the intersection of the level set $\set{x\in\Sigma}{f(x)=r}$ is a smooth curve.
\end{thm}
