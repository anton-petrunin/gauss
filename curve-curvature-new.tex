...
\section[\texorpdfstring{Total curvature $2\cdot\pi$}{Total curvature 2ṽ}]{Total curvature $\bm{2\cdot \pi}$}
...
Note that $\vec u$ is tangent indicatrix at point $p=\gamma(s)$ if and only if $p$ is a minimum point of $f$ on~$F$.
...
It follows that the map $\tan\:\mathbb{S}^1\to \mathbb{S}^1$ is \index{monotonic map}\emph{monotonic};
that is, for any $\vec u\z\in\mathbb{S}^1$, its inverse image $\tan^{-1}\{\vec u\}$ is a connected set in $\mathbb{S}^1$.
By \ref{ex:integral-length-0}, 
\[\length\tan=\length\mathbb{S}^1=2\cdot \pi.\]
Applying \ref{obs:tantrix}, we get $\tc\gamma=2\cdot \pi$ --- the if part is proved.
...
Let $\gamma$ be an open convex smooth curve.
Show that $\tc\gamma\le\pi$.
\end{thm}
...
Let $\gamma_1(s_0)$ be the point on $\gamma_1$ furthest to the line thru $\gamma(a)$ and $\gamma(b)$.
...
Moreover, the arc $\tan_1|_{[s,s_0]}$ lies in one of the arcs with endpoints $\vec u_1$ and $-\vec u_1$ in the unit circle. %??? more needed here
It follows that 
...
\]
Since the cosine is decreasing in the interval $[0,\pi]$, \ref{<gamma',u} implies 
