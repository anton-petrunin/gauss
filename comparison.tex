...
The following example shows that the statement \ref{SHORT.thm:comp:cat} does not hold without assuming that $\Sigma$ is simply-connected.
...
Intuitively the function $\bus_\lambda$ can be described as a distance function to an ideal point at infinity.
...
Then $\Sigma$ admits an intrinsic isometry to 
a circular cylinder $\set{(x,y,z)\in \mathbb{R}^2}{x^2+y^2=r^2}$ for some $r>0$ or
the Euclidean plane.
...
and it has a sequence of variations:
\begin{itemize}
...
Let $\Sigma$ be an open smooth surface with nonnegative Gauss curvature.
Suppose $\Sigma$ has a line and a half-line that meet at exactly one point.
