...
the minimizing geodesics $[x,y]_\Sigma$, $[y,z]_\Sigma$, and $[z,x]_\Sigma$ are called its {}\emph{sides};
...
Since the angles of any plane triangle add up to $\pi$, from the comparison theorems, we get that
\begin{itemize}
...
it will be deduced from the Rauch comparison (\ref{prop:rauch}).
...
\end{align*}
where $0$ is the origin in the tangent plane $\T_x$.
Note that $|\vec v|, |\vec w|< \tfrac r2$.
...
By the Rauch comparison (\ref{prop:rauch:K=<0}), we have
\[
\length \tilde \gamma \le \length \gamma.
\]
Since $\dist{\vec v}{\vec w}{\T_x} \le \length \tilde \gamma$ and $\length \gamma = \dist{y}{z}{\Sigma}$, it follows that
\[
\dist{\vec v}{\vec w}{\T_x} \le \dist{y}{z}{\Sigma}.
\]
By angle monotonicity (\ref{lem:angle-monotonicity}), we obtain
\[
...
Since $\Sigma$ is simply-connected, \ref{ex:unique-geod} implies that
\[\inj(p)_\Sigma=\infty\]
for any $p\in\Sigma$.
...
Let $\Sigma$ be an open or closed smooth surface with nonnegative Gauss curvature.
