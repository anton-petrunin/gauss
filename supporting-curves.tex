...
Then we say that $\gamma_1$ \index{supporting!curve}\emph{globally supports} $\gamma_2$ at point $p=\gamma_2(t_2)$ 
...
In this case, we say that $\gamma_1$ supports $\gamma_2$ \index{supporting!curve}\emph{from the inside} (\index{supporting!curve}\emph{from the outside}) if $\gamma_1$ supports $\gamma_2$ and lies in the region inside $\gamma_2$ (respectively outside $\gamma_2$). 
...
In order to get the osculating circles supporting $\gamma$ from the outside, one can repeat the proof of the key lemma taking instead of incircle the circline of maximal signed curvature that supports the curve from the outside, assuming that $\gamma$ is oriented so that the region on the left from it is bounded.
...
Therefore, these two circlines correspond to the osculating circlines supporting $\gamma$ from the outside.)
\qeds
...
Suppose a smooth simple plane loop $\gamma$ bounds area $a$.
...
%The following exercise is a version of four-vertex theorem for space curves without parallel tangents; they exist by \ref{ex:no-parallel-tangents}.

%\begin{thm}{Advanced exercise}\label{ex:4x0-torsion} Let $\gamma$ be a closed smooth space curve that has no pair of points with parallel tangent lines. Assume the curvature of $\gamma$ does not vanish. Show that $\gamma$ has four points with zero torsion. \end{thm}
