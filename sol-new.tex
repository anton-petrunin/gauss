...
Choose an inscribed polygonal line $\beta=p_0\dots p_{2\cdot k+1}$ such that $p_{2\cdot i}$ and $p_{2\cdot i+1}$ lie sufficiently close to $\gamma(s_i)$; so we can assume that the direction of $p_{2\cdot i+1}-p_{2\cdot i}$ is sufficiency close to $\vec u_i$ for each~$i$.
...
Use \ref{eq:|gamma(t)|} to show that the map $\Sigma\to \Theta$ defined by $x\mapsto 2\cdot\tfrac x{|x|}$ is locally invertable.
Conclude that it is invertbale.
...
Finally, we can move $\Sigma$ so that so that $x$ gets arbitrary close to $\Theta$.
Applying the above argument, we get that $\Sigma$ surronds a unit ball that touches $\Sigma$ at $x$.
