...
\parit{\ref{SHORT.ex:length-dist:self-intersection:>pi}.} Apply \ref{SHORT.ex:length-dist:>}.
...
Apply Sard's lemma (\ref{lem:sard}) to show that $\Norm$ is constant on $Z$, denote its value $\Norm_0$.
Apply Sard's lemma again to show that $x\mapsto \langle \Norm_0,x\rangle$ is constant on $Z$.
Make a conclusion.

\parit{Source.} It is a result of Richard Sacksteder \cite[Lemma 6]{sacksteder}.
