...
Use \ref{thm:length-semicont} to show that $s$ is lower semicontinous;
that is, if $t_n\to t_\infty$ as $n\to\infty$, then 
\[\liminf_{n\to\infty} s(t_n)\ge s(t_\infty).\]

Observe that
\[s(b)-s(t)=\length(\gamma|_{[t,b]}).\]
Apply \ref{thm:length-semicont} to show that $s$ is upper semicontinous.
Make a conclusion.

Instead of the second step, one can show and use that $s$ is nondecreasing.
