...
Applying a motion of the space,
we can assume that $p=f(p)$ is the origin,
the tangent planes to $\Sigma$ and $\Sigma^*$ at $p$ coincide with the $(x,y)$--plane,
the differential $d_pf$ is the identity map,
and both surfaces lie in the half-space $z\ge0$.

Using the bow lemma (\ref{lem:bow}), prove that $f$ maps any section of $\Sigma$ by a vertical plane through $p$ identically.
Deduce from this that $f$ is the identity map.

\parit{Source:} \cite[III § 6]{pogorelov}.
