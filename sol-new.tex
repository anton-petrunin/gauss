...
\parbf{\ref{ex:shrt=>continuous}.} Show that the conditions in \ref{def:continuous} hold for $\delta=\epsilon$.
...
Observe that $\gamma_{\vec u}$ runs in a semicircle $h$.
...
Since $f$ is a maximum or minimum at $0$, we get that $f'''(0)=0$.
Therefore
$\kur'=0$.
...
We can assume in addition that $\vec v_t$ vanishes outside a sufficiently large disc; this can be arranged by  multiplying the vector field by a function 
...
Therefore $\gamma$ is a curvature line in~$\Sigma_2$.
...
Therefore its signed curvature has constant sign.
...
\parbf{\ref{ex:saddle-convex}.} Suppose $p\in \Sigma$ is a point of strict  local maximum of $f$.
Arrive to a contradiction by showing that $\Sigma$ is supported by its tangent plane at $p$.


\parbf{\ref{ex:panov}.} Denote by $\Pi_t$ the tangent plane to $\Sigma$ at $\gamma(t)$ and by $\ell_t$ the tangent line to $\gamma$ at time $t$.
...
Suppose that the orthogonal projection of $\Sigma$ to the $(x,y)$-plane is not injective.
...
\parbf{\ref{ex:hat-convex}.} Suppose there is a minimizing curve $\gamma\not\subset\Delta$ with endpoints $p$ and $q$ in $\Delta$.
...
In the case when $\gamma$ is tangent to a meridian at some point, then by the  uniqueness part of Proposition \ref{prop:geod-existence},
...
\parbf{\ref{ex:milka}.} Let $W$ be the closed region outside $\Sigma$. Show that the distance $\dist{p^t}{q}W$ is constant with respect to $t$. This implies that the concatenation of the line segment $[p^t,\gamma(t)]$ and the arc $\gamma|_{[t,\ell]}$ is a shortest path from $p^t$ to $q$  in $W$. Since $\Sigma$ is strictly convex, 
...
By \ref{ex:two-min-geod}, $\gamma_1$ and $\gamma_2$ share only their endpoints.
...
\parbf{\ref{ex:cohn-vossen}.} Repeat the end of the proof of \ref{thm:cohn-vossen} for a one-parameter family of geodesics $\gamma_\tau$ defined by $\gamma_\tau(0)=\alpha(\tau)$ and $\gamma'(0)=\alpha'(\tau)$. 
...
To show that $\Delta_a$ is not congruent to a sphericsl disc $\Delta_1$ for $a\ne 1$, show that its principle curvature is not 1 at some point. 
