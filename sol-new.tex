...
By construction 
\begin{align*}
|\gamma(t_0)&-\gamma(t_1)|+|\gamma(t_1)-\gamma(t_2)|+\dots+|\gamma(t_{n-1})-\gamma(t_n)|=
\\
&=
...
\parbf{\ref{ex:tc(convex)}}; \ref{SHORT.ex:tc(convex):closed}.
Consider a closed convex curve $\gamma\:\mathbb{S}^1\to\mathbb{R}^2$.
Show that its tangent indicatrix $\tan\:\mathbb{S}^1\to\mathbb{S}^1$ is monotonic;
that is, for any $u\in\mathbb{S}^1$, the inverse image $\tan^{-1}\{u\}$ is a connected set in $\mathbb{S}^1$.
Then apply \ref{ex:integral-length-0} and \ref{obs:tantrix}. %???PIC

\parit{\ref{SHORT.ex:tc(convex):open}.}
Assume $\gamma\:\mathbb{R}\to\mathbb{R}^2$ is an open convex curve.
Show that the image of its tangent indicatrix of lies in a semicircle.
Then argue as in \ref{SHORT.ex:tc(convex):closed}.
