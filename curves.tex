\includepdf[pages=-]{Curves-draw.pdf}
\chapter{Definitions}
\label{chap:curves-def}

In most of the cases, by looking at tire tracks of a bicycle,
it is possible to say which direction it traveled and to distinguish the trails made by the front and rear wheels; plus one can find the distance between the wheels.
...
This question might motivate the reader to reinvent considerable part of the differential geometry of curves.
...
It turns out that any simple curve $\gamma$ can be \index{parametrization}\emph{parametrized} by a real interval or the circle.
...
If we say \index{curve}\emph{curve} it means we do not want to specify whether it is a parametrized curve or a simple curve.
...
\[\gamma'(t)=(x'(t),y'(t),z'(t))\] 
does not vanish at any point, then the parametrized curve $\gamma$ is called \index{regular!curve}\emph{regular}.
...
Any smooth regular closed curve can be described by a {}\emph{periodic} parametrized curve $\gamma\: \mathbb{R}\to \spc{X}$; that is, a curve such that $\gamma(t+\ell)=\gamma(t)$ for a fixed period $\ell > 0$ and all $t$.
...
\[\begin{cases}
  ...
  \end{cases}
\]
defines a regular smooth space curve if the set $\gamma$ of solutions is connected and $0$ is a regular value of the map $F\:\mathbb{R}^3\to\mathbb{R}^2$ defined as
